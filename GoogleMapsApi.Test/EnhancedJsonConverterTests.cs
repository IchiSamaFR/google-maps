...
namespace GoogleMapsApi.Test
{
    ...
    public class EnhancedJsonConverterTests
    {
        ...
        public void DurationJsonConverter_MissingProperties_DoesNotThrow()
        {
            ...
            Assert.DoesNotThrow(() => JsonSerializer.Deserialize<Duration>(json1));

            // Test missing text property  
            var json2 = """{"value": 3600}""";
            Assert.DoesNotThrow(() => JsonSerializer.Deserialize<Duration>(json2));

            // Test completely empty object
            var json3 = """{}""";
            Assert.DoesNotThrow(() => JsonSerializer.Deserialize<Duration>(json3));
        }
        ...
        public void DurationJsonConverter_ExtremeValues_HandledCorrectly()
        {
            ...
            var duration1 = JsonSerializer.Deserialize<Duration>(json1);
        ...
        public void EnumMemberJsonConverter_CachePerformance_Consistent()
        {
            ...
            var result1 = JsonSerializer.Deserialize<TravelMode>(json);
            
            // Multiple subsequent conversions - should use cache
            for (int i = 0; i < 100; i++)
            {
                var result = JsonSerializer.Deserialize<TravelMode>(json);
        ...
        public void EnumMemberJsonConverter_CaseVariations_Handled()
        {
            ...
            var result = JsonSerializer.Deserialize<TravelMode>(validJson);
            Assert.That(result, Is.EqualTo(TravelMode.Driving));

            // Test lowercase (should fail with Google API format)
            var lowercaseJson = "\"driving\"";
            var desrialized = JsonSerializer.Deserialize<TravelMode>(lowercaseJson);
        ...
        public void EnumMemberJsonConverter_NumericValues_OutOfRange()
        {
            ...
            var json = "999";
            Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<TravelMode>(json));

            // Test negative values
            var negativeJson = "-1";
            Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<TravelMode>(negativeJson));
        }

        [Test]
        public void EnumMemberJsonConverter_EmptyAndWhitespace_Strings()
        {
            Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<TravelMode>("\"\""));
            Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<TravelMode>("\" \""));
            Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<TravelMode>("\"\\t\""));
        }
        ...
        public void PriceLevelJsonConverter_BoundaryValues_Handled()
        {
            ...
            var json0 = "0";
            var result0 = JsonSerializer.Deserialize<PriceLevel?>(json0);
            Assert.That(result0, Is.EqualTo(PriceLevel.Free));

            // Test maximum valid value
            var json4 = "4";
            var result4 = JsonSerializer.Deserialize<PriceLevel?>(json4);
        ...
        public void PriceLevelJsonConverter_InvalidValues_ReturnNull()
        {
            ...
            var json5 = "5";
            var result5 = JsonSerializer.Deserialize<PriceLevel?>(json5);
            Assert.That(result5, Is.Null);

            var jsonNeg = "-1";
            var resultNeg = JsonSerializer.Deserialize<PriceLevel?>(jsonNeg);
            Assert.That(resultNeg, Is.Null);

            // Test invalid string
            var jsonInvalid = "\"invalid\"";
            var resultInvalid = JsonSerializer.Deserialize<PriceLevel?>(jsonInvalid);
        ...
        public void PriceLevelJsonConverter_FloatingPointNumbers_Handled()
        {
            ...
            var jsonFloat = "2.0";
            var result = JsonSerializer.Deserialize<PriceLevel?>(jsonFloat);
        ...
        public void OverviewPolylineJsonConverter_EmptyAndNullPoints_Handled()
        {
            ...
            var emptyResult = JsonSerializer.Deserialize<OverviewPolyline>(emptyJson);
            Assert.That(emptyResult, Is.Not.Null);

            // Test missing points property
            var missingJson = """{}""";
            var missingResult = JsonSerializer.Deserialize<OverviewPolyline>(missingJson);
        ...
        public void OverviewPolylineJsonConverter_LargePolylineData_HandlesCorrectly()
        {
            ...
            Assert.DoesNotThrow(() => 
            {
                var result = JsonSerializer.Deserialize<OverviewPolyline>(largeJson);
        ...
        public void OverviewPolylineJsonConverter_InvalidPolylineFormat_DoesNotCrash()
        {
            ...
            Assert.DoesNotThrow(() =>
            {
                var result = JsonSerializer.Deserialize<OverviewPolyline>(invalidJson);
                Assert.That(result, Is.Not.Null);
            });
        }
                ...
                tasks[i] = Task.Run(() =>
                {
                    try
                    {
                        for (int j = 0; j < 100; j++)
                        {
                            ...
                            JsonSerializer.Deserialize<Duration>(
                                """{"value": 3600, "text": "1 hour"}""");
                            
                            JsonSerializer.Deserialize<TravelMode>("\"DRIVING\"");
                            JsonSerializer.Deserialize<PriceLevel?>("2");
        ...
        public void AllConverters_MalformedJson_DoNotCrash()
        {
            ...
            foreach (var malformed in malformedInputs)
            {
                ...
                Assert.DoesNotThrow(() =>
                {
                    try
                    {
                        JsonSerializer.Deserialize<Duration>(malformed);
                    }
                    catch (JsonException)
                    {
